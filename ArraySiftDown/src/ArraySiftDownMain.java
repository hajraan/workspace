...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        int smallest = left;
        if (last >= right && array[left] > array[right]) {
            smallest = right;
        }

        if (last - top > 0) {
            // Compare smallest child to its parent
            if (last >= smallest && array[top] > array[smallest]) {
    ...
    private static <T> void heapify(int[] array, int top) {
        assert array != null : "Violation of: array is not null";
    ...
    public static void main(String[] args) {
        ...
        for (int i = heapSize; i < heapSize; i++) {
            array[i] = heapSize - i - 1;
        }
        /*
         ...
         */
        for (int i = heapSize; i < heapSize; i++) {
            assert heapSize - i - 1 == array[i] : ""
                    + "Method siftDown changed the junk at "
                    + "the far end of the array: Expected " + (heapSize - i - 1)
                    + " but was " + array[i];
        }
        /*
