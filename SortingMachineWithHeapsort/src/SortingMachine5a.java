...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        // used the method from the ArraySiftDown lab
        if (i != j) {
            T tmp = array[i];
            array[i] = array[j];
            array[j] = tmp;
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        int smallest = left;
        if (last >= right && order.compare(array[left], array[right]) > 0) {
            smallest = right;
        }
        ...
        if (last - top > 0) {
            ...
            if (last >= smallest
                    && order.compare(array[top], array[smallest]) > 0) {
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        if (heap.length > 1) {
            heapify(heap, heap.length / 2 - 1, order);
        }
        return heap;
    }
